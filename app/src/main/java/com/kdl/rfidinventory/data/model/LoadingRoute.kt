...
data class LoadingSummary(
    ...
    val fullBasketsScanned: Int,       // 完整格數
    val looseItemsScanned: Int,        // 散貨數量
    val totalScanned: Int,             // 總掃描數量
    val totalExpected: Int,            // 總期望數量
    val isComplete: Boolean,           // 是否完成
    val scannedBaskets: List<LoadingScannedItem>
)

/**
 * 擴展函數：計算期望的籃子數量
 */
fun LoadingItem.getExpectedBaskets(product: Product?): Int {
    return fullTrolley * 5 + fullBaskets
}

/**
 * 擴展函數：計算期望的完整籃子總數量
 */
fun LoadingItem.getExpectedFullBasketQuantity(product: Product?): Int {
    val maxCapacity = product?.maxBasketCapacity ?: 20
    return (fullTrolley * 5 + fullBaskets) * maxCapacity
}
